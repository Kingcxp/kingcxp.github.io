...
我的建议是：**两个都装**。大部分时候用 `MinGW` 就够，但遇到需要调用 `Windows API` 或某些只提供 `MSVC` 接口的库时（比如 `DirectX`），`MSVC` 就派上用场了。而且两者并不冲突，可以共存。
...
| **Code Runner** | 一键运行代码片段，适合初学者快速测试 |
...
`Zed` 是一款新兴的高性能编辑器，由 `Atom` 和 `Tree-sitter` 的原班人马开发，启动速度和编辑响应都非常快。目前支持 `macOS` 和 `Linux`，`Windows` 版本尚在开发中。
...
- **Zed**：启动更快、响应更流畅，适合追求编辑体验的进阶用户，但目前 `Windows` 用户暂时无法使用。
:::
...
- 常用编译选项：`-Wall`（警告）、`-g`（调试）、`-O2`（优化）、`-std=c99`（语言标准）
