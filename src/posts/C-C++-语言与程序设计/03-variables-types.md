...
- **名字**：你叫它的方式，比如 `radius`、`pi`
- **类型**：这个盒子里能放什么形状的数据，比如整数、小数、字符
...
直接使用没有被初始化的变量是一种“未定义行为”，未定义行为表示在编译器中没有明确定义执行效果的行为，包含该行为编译出来的程序运行时可能会出现任何结果，甚至程序崩溃。

因此，为了避免这种情况，养成良好的习惯：**声明变量时就初始化**。
:::
...
:::tip
`%zu` 是专门用来输出 `sizeof` 结果的格式符，因为 `sizeof` 返回的类型是 `size_t`。如果你在比较老的编译器上遇到警告，可以试试 `%lu` 并把结果强转成 `unsigned long`。
:::
...
默认情况下，`int` 是**有符号**的（`signed`），即它可以表示正数和负数。你也可以用 `unsigned` 关键字声明**无符号**整型，它只能表示非负数，但正数的范围更大。
...
有符号整型用**最高位**（最左边的一位）作为符号位：0 表示正数，1 表示负数。剩下 31 位（以 32 位 `int` 为例）用来存数值。

那么具体数值是怎么算的呢？现代计算机一律采用**补码**（two's complement）来表示有符号整数。补码的规则很简单：

- **正数**：直接转成二进制，符号位为 0。比如 `+42` 就是 `0b000...00101010`。
- **负数**：先取绝对值的二进制，然后**按位取反（所有 0 变 1、1 变 0），再加 1**。
...
因为补码表示中，`0` 只有一种编码（所有位都是 0）。如果不把 `0` 算两次，那么对于 32 位来说，总共 2^32 种组合，有一半（2^31 个）用来表示非负数（`0` 和正数），另一半（2^31 个）用来表示负数。所以正数的最大值是 2^31 - 1（因为要留一个给 0），而负数的最小值是 -2^31。这就是为什么 `int` 的范围是 **-2,147,483,648 ~ 2,147,483,647**，负数比正数多了一个。
...
`float` 和 `double` 采用 **IEEE 754** 标准来存储小数。它的思想类似于科学记数法：
...
`bool` 只有 `true`（1）和 `false`（0）两种状态，理论上 1 个二进制位就够了。但在 `C` 语言中，`bool` 占用 **1 个字节（8 位）**。原因有两个：

1. **最小寻址单位**：CPU 和内存之间传输数据的最小单位是字节（byte），而不是位（bit）。你无法单独请求"读取内存中的第 3 位"——一次至少读一个字节。
2. **对齐与性能**：按字节对齐访问比按位操作快得多。用一个字节存一个布尔值，CPU 可以直接读写；如果用 1 位，每次都需要额外的"打包/拆包"操作，反而更慢。

所以，这是一次典型的**用空间换时间**——多花 7 位的空间来换取更快的访问速度。

### 字符类型与 ASCII 表

`char` 在底层其实就是一个 **8 位整数**，它的值对应着 ASCII 编码表中的字符。
...
**默认的 `char` 是有符号的**。在大多数编译器上，`char` 相当于 `signed char`，取值范围是 **-128 ~ 127**。而标准 ASCII 的范围是 0 ~ 127，有符号 `char` 刚好够用。但有些扩展字符集（如 Latin-1）会用到 128 ~ 255 的值，如果 `char` 是有符号的，`char c = 200;` 会被解释成负数，导致意想不到的结果。

因此，**如果 `char` 的主要用途是存储字符**，建议使用 `unsigned char`，取值范围是 0 ~ 255，能覆盖更大的字符集。**除非你明确需要把 `char` 当作 8 位有符号整数来使用**（比如处理 -128 ~ 127 之间的数值），才保留默认的有符号 `char`。

```c
char c = 200;              // 可能溢出为负数（-56）
unsigned char uc = 200;    // 正确存储 200
printf("%d %d\n", c, uc);  // -56 200
```

## 字面量

代码里直接写出来的值就叫做**字面量**。比如：
...
- 写 `float` 字面量时，最好加上 `f` 后缀，比如 `3.14f`。不加的话编译器可能默认当成 `double`，再转换回 `float`，虽然 usually 没问题，但加上更明确。
- 字符字面量用单引号 `'a'`，字符串字面量用双引号 `"hello"`，这两个不能混用。

## 常量：#define 和 const

有些值在程序运行期间不应该被修改，比如圆周率 π、数组的最大长度等。这时我们可以用**常量**。
...
**`#define` 宏常量**
`#define` 在编译开始前就已经完成了文本替换。代码中的 `PI` 被直接替换成了 `3.14159`，这个值作为立即数直接出现在指令中，程序运行时**根本不需要再去内存中读取**。因此从理论上说，`#define` 完全没有运行时的内存访问开销，速度最快。

用一个生活类比来理解：

- **普通变量**：你把一个数字写在便利贴上，贴在冰箱上。每次要用都得跑过去看一眼。
- **`const` 常量**：你把数字写在便利贴上，又用透明胶带牢牢粘住。要用还是得跑去看，但聪明的编译器有时会说"我记得这个数，你不用跑了"。
- **`#define`**：你把数字直接刻在每件工具上。要用的时候低头就能看见，根本不用跑。

不过在实际开发中，这种速度差异通常微乎其微。除非在极端性能敏感的场景（如嵌入式系统、游戏引擎的核心循环），否则大可不必为此纠结。**代码的可读性和正确性远比这种微小的性能差异重要。**
:::

## printf 与 scanf 的格式占位符

`printf` 和 `scanf` 是 `C` 语言最常用的输入输出函数。它们本身并不清楚你要输出什么类型的数据，所以需要靠**格式占位符**来告诉它们。
...
还记得我们一开始把变量比作"盒子"吗？每个盒子都摆在内存的某个位置上，都有一个独一无二的位置编号，这就是**地址**。

`C` 语言中，函数参数默认是**按值传递**的——也就是说，当你调用 `scanf("%lf", x)` 时，传入的是盒子里装的那个数值，而不是盒子本身。`scanf` 拿到数值后，虽然想"往这个盒子里写入新数据"，但它手上只有数值，不知道盒子在哪儿，所以没办法把新数据放进去。

`&` 符号的作用就是**取地址**，即获取变量这个盒子的位置编号。你告诉 `scanf` 的是 `&x`（盒子 `x` 的位置编号），`scanf` 就能顺着这个编号找到盒子 `x` 所在的位置，直接把读到的数据放进去。

用更形象的比喻来说：

> 变量 `x` 就像一个带编号的信箱。`x` 的值是信箱里当前装的信件。`&x` 就是信箱的门牌号。如果你只把信件交给 `scanf`，它看完就扔了，没法把新信放回你的信箱。但如果你把门牌号告诉 `scanf`，它就能直接走过去，打开信箱，把新信放进去。

所以记住：**凡是要通过 `scanf` 往变量里写入数据的，基本都要加 `&`**。
...
| 风格 | 示例 | 常见使用场景 |
| :-- | :-- | :-- |
| **snake_case（蛇形命名）** | `student_name`、`max_value` | C 语言标准库、Linux 内核、Python |
| **camelCase（小驼峰）** | `studentName`、`maxValue` | Java、JavaScript、部分 C++ 项目 |
| **PascalCase（大驼峰）** | `StudentName`、`MaxValue` | C++/Java 类名、结构体名 |

### C 语言：基本只用 snake_case

C 语言社区的风格非常统一：**变量、函数、结构体类型几乎都用 snake_case**。唯一的例外是宏常量，习惯全大写加下划线：
...
- **变量和函数**：标准库偏爱 snake_case（`std::vector`、`std::find_if`），但很多项目用 camelCase（`getStudentName`）。
- **成员变量**：有人喜欢加 `m_` 前缀（`m_name`），有人喜欢下划线后缀（`name_`），也有人什么都不加。
- **宏常量**：和 C 一样，全大写加下划线。
...
下一篇文章，我们会专门聊聊输入输出，把 `printf` 和 `scanf` 的用法彻底搞清楚。
