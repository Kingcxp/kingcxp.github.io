...
对于浮点成员，直接比较是否合理取决于类型语义。数学近似向量可能需要显式 `almost_equal`，而作为哈希键或精确状态时又可能需要严格相等。不要把误差策略偷偷藏进所有 `==`。
...
后置版本通常需要保留旧值，因此可能更昂贵。迭代器等类型在不需要旧值时优先使用前置自增。
...
文件句柄、互斥量、网络连接等对象表示独占资源或身份，复制含义不自然。可以禁止复制，只允许移动：
