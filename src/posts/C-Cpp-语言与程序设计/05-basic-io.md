...
int age;
if (scanf("%d", &age) != 1) {
    fprintf(stderr, "请输入合法整数\n");
    return 1;
}
```

不检查返回值就继续使用 `age`，可能会读到未初始化值或旧值。

## 为什么推荐“先读一行，再解析”

<span style="color: #F56C6C;">`scanf` 把格式解析和流读取绑在一起，遇到错误输入后，未被消费的字符还留在输入流中，容易让下一次读取再次失败。</span>较稳妥的交互程序常采用：
...
if (errno != 0 || end == line || value < INT_MIN || value > INT_MAX) {
    fprintf(stderr, "输入不是 int 范围内的整数\n");
    return 1;
}
```

这里可以分别判断“是否读到一行”“是否有数字”“是否越界”“数字后是否还有多余字符”。

## 输出也可能失败

文件写满、管道关闭时，输出函数也可能失败。小练习通常不必处理每一次 `printf` 返回值，但写文件或网络协议时必须检查错误。输入输出函数是与外部世界交互的接口，不应假定外界永远按预期工作。
