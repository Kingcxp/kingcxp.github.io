...
最后一个阶段项目把现代 C++ 的值语义、标准容器、算法、lambda、文件资源和错误处理组合起来。目标是做一个命令行任务管理器，而不是再手动实现一遍动态数组和字符串。
...
字段：ID、完成状态、优先级、标题。需要规定标题中的制表符和换行如何转义。
...
此时可以使用 CMake 创建核心库、主程序和测试程序三个目标。下一篇调试章节会说明如何在这样的多文件项目中使用调试器定位问题。
