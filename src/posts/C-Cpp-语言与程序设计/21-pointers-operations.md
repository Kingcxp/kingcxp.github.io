...
<span style="color: #67C23A;">我推荐 `int *p;`，因为它更清楚地表达“`*p` 是一个 `int`”。</span>
...
`*` 放在指针前面，表示“取出这个地址里的内容”，也叫<span style="color: #409EFF;">解引用</span>。
...
关键理解：`*p` 和 `a` 在这里是<span style="color: #409EFF;">同一个内存位置的两种叫法</span>。你改 `*p`，就是改 `a`。
...
1. **接口自文档化**——看到 `const int *`，调用者就知道这个函数不会修改传入的数据。
2. **编译器帮你检查**——如果你在函数里不小心写了尝试修改的代码，编译器会直接报错，而不是让你在运行时排查。
3. **更安全**——防止无意中改掉不该改的数据。
...
比 `NULL` 更危险的是<span style="color: #F56C6C;">野指针</span>：它指向一个未知、无效、甚至已经被释放的地址。
...
1. <span style="color: #F56C6C;">未初始化就解引用</span>。`int *p; *p = 10;` 是典型的野指针错误。
2. <span style="color: #F56C6C;">解引用 NULL 指针</span>。`int *p = NULL; printf("%d", *p);` 会崩溃。
3. <span style="color: #F56C6C;">混淆 `*` 在声明和表达式中的含义</span>。声明里 `int *p` 表示 p 是指针；表达式里 `*p` 表示取内容。
4. <span style="color: #F56C6C;">把整数直接赋给指针</span>。`int *p = 100;` 只在极少数底层场景有意义，初学不要这么写。
...
<span style="color: #F56C6C;">`NULL` 不等于悬空指针，也不等于未初始化指针。</span>空指针可比较、可传递、可 `free`，但不能解引用。接口允许空值时要写清语义；不允许时应在入口检查或用文档建立前置条件。
