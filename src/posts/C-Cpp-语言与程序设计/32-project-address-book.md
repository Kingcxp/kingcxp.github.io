...
不要一开始就加入登录、网络同步、图形界面等功能。综合项目的目标是练习清晰边界，而不是无限堆需求。
...
这里用末尾元素覆盖被删位置，删除是 `O(1)`，但会改变顺序。如果必须保持顺序，就要移动后续元素。
...
这个项目把“数据、所有权、文件格式、错误传播、模块边界”放在一起。后面的动态数组章节会把其中的扩容逻辑抽象成更通用的数据结构。
