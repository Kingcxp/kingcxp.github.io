...
函数重载已经在 C++ 函数接口章节介绍。本篇只处理继承层次中三个容易混淆的概念：覆盖、隐藏，以及返回类型的特殊规则。
...
基类写一次 `virtual` 后，派生层次中对应覆盖仍然是虚函数，即使省略 `virtual`。但推荐在派生类使用 `override`。
...
隐藏发生在名字查找阶段，不要求基类函数是虚函数，也不等于覆盖。
