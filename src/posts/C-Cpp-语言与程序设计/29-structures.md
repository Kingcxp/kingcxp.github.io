...
`sizeof(struct Example)` 可能大于两个成员大小之和，因为编译器会插入填充以满足对齐。不要把结构体内存直接当成稳定的文件或网络格式；不同 ABI、编译选项和平台的布局可能不同。

协议序列化应逐字段编码，并明确字节序和宽度。
...
若成员都是整数和内嵌数组，结构体赋值会复制这些成员。若成员含指针，只会复制指针值，不会自动复制其指向的数据。这与后面 C++ 浅拷贝问题是同一根源。
...
按值传递会复制整个结构体；指针传递避免复制，并通过 `const` 表明不修改。小型结构体按值传递也完全合理，选择取决于语义和成本，而不是死记“结构体必须传指针”。
...
C99 指定初始化不依赖成员书写顺序，新增字段时也更不容易把值放错位置。
